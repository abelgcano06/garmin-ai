"""
generate_sleep_brief.py
=======================
Genera sleep_brief_ai.json para una fecha específica usando solo los
archivos JSON ya existentes en disco — no necesita cliente Garmin.

Uso:
    python generate_sleep_brief.py --date 2026-04-19
...
def main():
    ...
    if not os.path.exists(analysis_path):
        print(json.dumps({"ok": False, "error": f"sleep_analysis.json no encontrado: {analysis_path}"}))
        sys.exit(1)

    # Check cache
    history_dir = get_sleep_history_dir(user_id)
    sleep_day_dir = os.path.dirname(analysis_path)
    brief_ai_path = os.path.join(sleep_day_dir, "sleep_brief_ai.json")
    ...
    result = generate_sleep_ai_brief(
        analysis_path=get_sleep_analysis_path(date_str, user_id),
        ...
        longitudinal_brief_path=os.path.join(history_dir, "sleep_longitudinal_brief.json"),
        trends_path=os.path.join(history_dir, "sleep_trends.json"),
        patterns_path=os.path.join(history_dir, "sleep_patterns.json"),
        user_dir=user_dir,
    )
